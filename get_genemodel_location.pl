...
my $filein_protids="/home/mnguyen/Research/Symbiomics/AA9/Annie/aa9_targets_JGI_protIDs_04Jun2018.txt";
my $folderin_gff="/home/mnguyen/Research/JGI_Mycocosm/All_GFF_files/GeneCatalog";
my $fileout="/home/mnguyen/Research/Symbiomics/AA9/Annie/aa9_targets_JGI_protIDs_04Jun2018_gene_location.txt";
open(Out,">$fileout") || die "Cannot open file $fileout";
print Out "#Protid\tScaffold\tStrand\tGenemodel\tStartcodon\tStopcodon\n";
...
sub Process_GFF
{
	...
	while (<GFF_file>)
	{
		...
		if ($feature eq "CDS")
		{
			if ($info=~/name\s*\"(.+)\"\;\s*proteinId\s*(\d+)\;\s*(exonNumber\s*\d+)/)#name "estExt_fgenesh1_pg.C_1_t10003"; proteinId 445593; exonNumber 1
			{
				my $genemodel_name=$1;
				my $protein_id=$2;
				my $exon_number=$3;
			...
			}else{print "Error (".__LINE__."): protein id information in gff file is not as described\n$info\n";exit;}
		}
	...
	{
		print Out "jgi\|$species\|$protid\t$hash_protid_scaffold{$protid}\t$hash_protid_strand{$protid}\t$hash_protid_genemodel{$protid}\t$hash_protid_startcodon{$protid}\t$hash_protid_stopcodon{$protid}\n";
	}
	close(GFF_file);
}
#========================================================================================================================#
...
sub Process_GFF3
{
	...
	my %hash_protids;
	foreach my $protid (@all_protids){$hash_protids{$protid}++;}
	#while (my ($k, $v)=each(%hash_protids)){print "$k\t$v";}exit;
	...
	while (<GFF3_file>)
	{
		...
		if ($_!~/^#/)
		{
			...
			if ($feature eq "mRNA")
			{
				if ($info=~/proteinId\=(\d+)/)#name "estExt_fgenesh1_pg.C_1_t10003"; proteinId 445593; exonNumber 1
				{
					$protein_id=$1;
					#print "\n-$protein_id-\n";exit;
					if ($hash_protids{$protein_id})
